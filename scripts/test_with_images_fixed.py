        ...
        try:
            if not tracking_active:
                ...
                if boxes and len(boxes) > 0:
                    ...
                    if (
                        area_coverage < 0.95 and bbox.confidence > 0.20
                    ):  # Lowered from 0.25
                        ...
                        if state:
                            ...
                            # Track not established - this is OK for first frame, Bot-SORT needs multiple detections
                            # Try again on next frame
                            logger.debug(
                                f"  {img_file.name}: Track not yet established (will retry)"
                            )
                ...
                # Continue tracking - Bot-SORT should maintain track
                state = model.update(image)
                if state:
                    logger.info(
                        f"  {img_file.name}: ✓ Tracking - "
                        f"x={state.bbox[0]:.0f}, y={state.bbox[1]:.0f}, "
                        f"conf={state.confidence:.3f}"
                    )
                    results.append(True)
                    ...
                    # For image sequences, re-detect every frame if tracking is lost
                    logger.debug(
                        f"  {img_file.name}: Tracking lost, attempting re-detection..."
                    )
                    boxes = model.detect(image, prompt)
                    if boxes and len(boxes) > 0:
                        bbox = boxes[0]
                        h, w = image.shape[:2]
                        area_coverage = (bbox.width * bbox.height) / (w * h)
                        # Accept detections with reasonable coverage (including small objects)
                        if area_coverage < 0.95 and bbox.confidence > 0.20:
                            initial_bbox = (bbox.x, bbox.y, bbox.width, bbox.height)
                            state = model.update(image, initial_bbox=initial_bbox)
                            if state:
                                logger.info(
                                    f"  {img_file.name}: ✓ Re-detected and tracking resumed"
                                )
                                results.append(True)
                            else:
                                # Even after re-detection, track not established
                                # This might be OK if we're still in establishment phase
                                logger.warning(
                                    f"  {img_file.name}: ✗ Re-detection failed to establish track"
                                )
                                tracking_active = False
                                results.append(False)
                        else:
                            logger.warning(
                                f"  {img_file.name}: ✗ Re-detection rejected (coverage={area_coverage:.2f}, conf={bbox.confidence:.3f})"
                            )
                            tracking_active = False
                            results.append(False)
                    else:
                        logger.warning(
                            f"  {img_file.name}: ✗ Re-detection found nothing"
                        )
                        tracking_active = False
                        results.append(False)
        except Exception as e:
