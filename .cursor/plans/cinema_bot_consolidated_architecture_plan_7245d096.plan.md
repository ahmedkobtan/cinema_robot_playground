...
overview: Comprehensive consolidated architecture plan addressing all four proposals, user feedback, and research gaps. Includes validated model selection (Follow Anything, Grounding DINO, SAM 2, Bot-SORT), VLA model evaluation (OpenVLA, RT-2), agentic AI architecture with LangChain ReAct, visual servoing design with movement hierarchy, moving object handling, and validated cinematic shot types.
todos:
  - id: validate-models
    content: Benchmark Follow Anything (FAn), Grounding DINO, SAM 2, and Bot-SORT on RTX 2080 Ti to verify performance claims and VRAM usage
    status: pending
  - id: prototype-tracking
    content: "Implement prototype tracking pipeline: Follow Anything (FAn) standalone OR Grounding DINO → SAM 2 → Bot-SORT for open-vocabulary object tracking"
    status: pending
    dependencies:
      - validate-models
  - id: arduino-firmware
  ...
  - id: video-pipeline
    ...
    status: completed
  - id: director-agent
    content: "Implement Director Agent (Layer 1): LangChain ReAct agent with LLM (Llama 3.1 8B via Ollama) for agentic command parsing, integrated with Grounding DINO or FAn for object detection"
    status: completed
    dependencies:
      - prototype-tracking
  - id: visual-servoing
